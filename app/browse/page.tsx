  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm text-slate-500 dark:text-slate-400">{questions.length} questions total</span>
      </div>

      <div className="flex flex-wrap gap-2">
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search text or question ID…" className="flex-1 rounded-md border border-slate-300 dark:border-slate-700 px-3 py-2 text-sm" />
        ...
        <select value={status} onChange={(e) => setStatus(e.target.value as any)} className="rounded-md border border-slate-300 dark:border-slate-700 px-3 py-2 text-sm">
              ...
              <button onClick={() => setExpanded(expanded === q.id ? null : q.id)} className="flex w-full items-start justify-between gap-3 text-left">
                <div className="min-w-0">
                  <div className="mb-1 flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-xs text-slate-500 dark:text-slate-400">{q.skill}</span>
                    {flaggedSet.has(q.id) && <span className="text-xs text-amber-600">🚩</span>}
                    {l && <span className={`text-xs ${l.is_correct ? "text-emerald-600 dark:text-emerald-300" : "text-rose-600 dark:text-rose-400"}`}>{l.is_correct ? "✓ correct" : "✗ wrong"}</span>}
                    {!l && <span className="text-xs text-slate-400 dark:text-slate-500">unattempted</span>}
                    <span className="font-mono text-[11px] text-slate-300">{q.question_id}</span>
                  </div>
                  <p className="line-clamp-2 text-sm text-slate-700 dark:text-slate-200">{q.question_text}</p>
                </div>
                <span className="flex-none text-slate-400 dark:text-slate-500">{expanded === q.id ? "−" : "+"}</span>
              </button>
              ...
              {expanded === q.id && (
                <div className="mt-3 border-t border-slate-100 pt-3 text-sm">
                  {q.graph_url ? (
                    ...
                    <img src={q.graph_url} alt="Question graphic" className="mb-3 max-w-full rounded-lg border border-slate-200 dark:border-slate-800" />
                  ) : q.needs_graphic ? (
                    <p className="mb-3 rounded-md bg-amber-50 dark:bg-amber-950 px-3 py-2 text-xs text-amber-700 dark:text-amber-300">
                      ⚠ This question relies on a graph that isn&apos;t available yet, so it&apos;s hidden from practice.
                    </p>
                  ) : null}
                  <QuestionText text={q.question_text} underlineSpans={q.underline_spans} className="text-slate-700 dark:text-slate-200" />
                  {q.choices && (
                    <ul className="mt-3 space-y-1">
                      {q.choices.map((c) => (
                        <li key={c.letter} className={c.letter === q.correct_answer ? "font-medium text-emerald-700 dark:text-emerald-300" : "text-slate-600 dark:text-slate-300"}>
                          {c.letter}. {c.text}{c.letter === q.correct_answer && " ✓"}
                        </li>
                      ))}
                    </ul>
                  )}
                  <p className="mt-3 text-sm"><span className="font-semibold text-slate-700 dark:text-slate-200">Correct answer:</span> <span className="text-emerald-700 dark:text-emerald-300">{q.correct_answer}</span></p>
                  {note && <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">📝 {note}</p>}
                  {q.rationale && (
                    <div className="mt-3 rounded-md bg-slate-50 dark:bg-slate-800 p-3">
                      ...
                      <p className="whitespace-pre-wrap text-slate-600 dark:text-slate-300">{q.rationale}</p>
                    </div>
                  )}
                </div>
              )}
      ...
      {filtered.length > 300 && <p className="text-center text-xs text-slate-400 dark:text-slate-500">Showing first 300 — refine your search to see more.</p>}
    </div>
  );
}
